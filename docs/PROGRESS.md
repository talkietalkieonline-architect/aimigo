...
### Что следующее
1. Push в GitHub (нужен новый токен)
2. Подключение FastAPI к PostgreSQL
3. API авторизации (телефон + SMS + пароль)
4. Мои агенты (модалка с группами)
5. Город агентов (каталог + поиск + фильтры)
6. Рабочий чат с вводом текста
