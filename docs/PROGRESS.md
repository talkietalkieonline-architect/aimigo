...
**1. ЛК Бизнеса — полный UI настройки персонажа:**
   - Голос (выбор из библиотеки, скорость, тон)
   - Внешность (лицо, причёска, кожа)
   - Одежда (стиль, верх, низ, обувь, аксессуары — потом шкаф)
   - Манеры (стиль общения, темперамент, юмор)
   - Знания (текстовая база, загрузка данных)

**2. Личный агент пользователя (по подписке):**
   - При подписке пользователь получает агента-болванку (citizen)
   - Видно в «Мои агенты» → строка «Личные»
   - Нажатие → меню с кнопкой «Настройки»
   - Панель настроек аналогична бизнесу:
     голос, внешность, одежда, манеры, знания

**3. Сценарий приветствия Дворецкого:**
   - Новый пользователь: «Добро пожаловать...» + TTS озвучка
   - Возврат существующего: «Привет, {name}! Чем могу помочь?»
   - Не повторять одно и то же каждый раз

**4. Микрофон — проверка логики:**
   - off = не слушает
   - on (подсвечен) = слушает и транскрибирует
   - Проверить на реальном устройстве

**5. Docker compose up** — проверка полной связки

**6. TTS** для ответов агентов
